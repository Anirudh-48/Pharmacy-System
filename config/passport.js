            ...
            function (req, username, password, done) {
                ...
                connection.query("SELECT * FROM login WHERE username = ?", [username], function (err, rows) {
                    ...
                    if (rows.length) {
                        ...
                        // connection.connect();
                        connection.query(insertQuery, [newUserMysql.username, bcrypt.hashSync(newUserMysql.password,null,null)], function (err, rows) {
    ...
    passport.use(
        ...
        new LocalStrategy({
            ...
            function (req, username, password, done) { // callback with email and password from our form
                // connection.connect();
                connection.query("SELECT * FROM login WHERE login.username = ? and login.password= ? or login.username = ? and login.password= ? ", [username,password,username,bcrypt.hashSync(password,null,null)], function (err, rows) {
                    if (err)
                        return done(err);
                    if (!rows.length) {
                        // console.log(rows)
