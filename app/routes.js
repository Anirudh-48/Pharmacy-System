...
module.exports = function (app, passport) {
  ...
  app.get("/", isLoggedIn, function (req, res) {
    ...
    var query2="select patient_1.*,login.username from patient_1 inner join login on patient_1.login_id=login.login_id where patient_1.login_id= ?";
    connection.query(query2,[req.user.login_id], function(err,rows)
    {
  ...
  app.post("/updatePatient",function(req,res){
    ...
    {
      var temp="update login set username=? where username=?";
      connection.query(temp,[username,old_username],(err,row)=>{
        if(err) console.log(err);
        console.log(row);
      })
      console.log(req.body);
     ...
     if(req.body.Address)
    {
      var query="update patient_1 set address=? where pat_id=?";
      connection.query(query,[address,pat_id],(err,row)=>{
        // err?console.log(err):"";
      })
    }
     if(req.body.Contact)
    {
      var query="update patient_1 set contact=? where pat_id=?";
      connection.query(query,[contact,pat_id],(err,row)=>{
        // err?console.log(err):"";
      })
    }
    res.redirect("/")
    ...
    var query =
      "SELECT bill_1.bill_no, bill_1.payment_mode, bill_1.discount, bill_1.total_cost, \
		DATE_FORMAT(bill_1.bill_date,'%d-%m-%Y') AS bill_date, patient_1.pat_name\
		 FROM bill_1 INNER JOIN patient_1 ON bill_1.pat_id=patient_1.pat_id ORDER BY bill_1.bill_no DESC";
    connection.query(query, function (err, rows) {
